...
(function initAboutProfile() {
  const input = document.getElementById('aboutProfileInput');
  const preview = document.getElementById('aboutProfilePreview');
  const frame = document.getElementById('aboutProfileFrame');
  const storageKey = 'about_profile_image';
  ...
  function setPreview(src) {
    ...
    setTimeout(() => { preview.style.transform = 'scale(1)'; }, 280);
    // persist
    try { localStorage.setItem(storageKey, src); } catch (err) { /* ignore */ }
  }
  ...
  // clicking the frame opens file dialog
  ...
  frame.addEventListener('drop', (ev) => {
    ...
    if (f && f.type.startsWith('image/')) {
      // try to set input.files for compatibility
      try { input.files = ev.dataTransfer.files; } catch (err) {}
      handleFile(f);
    }
  });
})();
